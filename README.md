# bunnylife-rulespanel
Uso Autorizado – BunnyLife Rules Panel


Este sistema, denominado BunnyLife Rules Panel, está destinado exclusivamente para uso autorizado de Samuel Landau y de la comunidad Bunny Life.
Queda estrictamente prohibido el acceso, copia, distribución, modificación o utilización de este software por parte de personas no autorizadas.
Cualquier uso indebido, intento de acceso no autorizado o reproducción total o parcial del contenido implicará la aplicación de las medidas correctivas correspondientes.

El BunnyLife Rules Panel forma parte de la infraestructura interna de Bunny Life y debe ser utilizado únicamente para fines operativos, administrativos y de gestión dentro del entorno autorizado

# Bunny Life - Sistema de Normativas

Sistema completo de gestión de normativas para servidor de FiveM Roleplay, desarrollado con arquitectura modular que combina una interfaz web moderna con un backend robusto y un bot de Discord integrado.

## Descripción

Bunny Life es una plataforma web profesional diseñada para gestionar y visualizar normativas de un servidor de roleplay. El sistema permite a los administradores crear, editar y organizar contenido de manera dinámica, mientras que los usuarios pueden acceder a las normativas de forma pública y realizar búsquedas eficientes.

### Frontend

[![Next.js](https://img.shields.io/badge/Next.js-000000?logo=nextdotjs\&logoColor=white)]()
[![React](https://img.shields.io/badge/React-20232A?logo=react\&logoColor=61DAFB)]()
[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript\&logoColor=white)]()
[![Tailwind](https://img.shields.io/badge/TailwindCSS-38B2AC?logo=tailwindcss\&logoColor=white)]()
[![shadcn/ui](https://img.shields.io/badge/shadcn/ui-000000?logo=shadcnui\&logoColor=white)]()
[![Lucide](https://img.shields.io/badge/Lucide-000000?logo=lucide\&logoColor=white)]()

### Backend

[![Node.js](https://img.shields.io/badge/Node.js-43853D?logo=node.js\&logoColor=white)]()
[![Express](https://img.shields.io/badge/Express-000000?logo=express\&logoColor=white)]()
[![SQLite](https://img.shields.io/badge/SQLite-07405E?logo=sqlite\&logoColor=white)]()
[![JWT](https://img.shields.io/badge/JWT-000000?logo=jsonwebtokens\&logoColor=white)]()
[![bcryptjs](https://img.shields.io/badge/bcryptjs-orange?logo=javascript\&logoColor=white)]()
[![CORS](https://img.shields.io/badge/CORS-005B9A?logo=security\&logoColor=white)]()

### Integraciones

[![Discord.js](https://img.shields.io/badge/Discord.js-5865F2?logo=discord\&logoColor=white)]()
[![Express (HTTP)](https://img.shields.io/badge/Express%20API-000000?logo=express\&logoColor=white)]()

### Pantalla de carga
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/4f73b226-447c-4ee1-a717-b2ef3d5bff39" />

### Inicio 
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/39373289-3d4f-4632-b5c4-5f83dd22d162" />

### Login
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/68179f17-33ac-4c5d-8f02-84f02032100b" />

### Inicio administrativo
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/6a90f548-485b-46d9-97c5-7503e612e7b6" />

### Selectores por colores al modificar
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/ff52d091-34bf-448f-a6a0-6bb0f91f5289" />

### Sistema de gestión de usuarios
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/17648a38-6573-4484-826f-4ab136fa9fc1" />

### Sistema de logs & cambios con restauración
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/ba070cac-b29c-4013-9bd8-19a0d1a8f3d7" />

### Sistema de roles y permisos 
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/faf33d5e-b651-4ea7-9e0a-f7d2a3e12a11" />

### Backup & restore
<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/fb602c5f-ca1e-4d46-b87a-a00d6d6b7cf0" />
